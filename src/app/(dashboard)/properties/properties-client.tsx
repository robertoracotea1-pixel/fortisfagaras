...
function PropertyCard({ p }: { p: Property }) {
  ...
  const primaryUrl = p.sources[0]?.url;
          ...
          <div className="flex items-start justify-between gap-2">
            <div>
              <p className="text-base font-bold text-foreground">
                {p.price > 0 ? `${p.price.toLocaleString("ro")} ${p.currency}` : "Preț la cerere"}
              </p>
...
export function PropertiesClient({ properties }: { properties: Property[] }) {
  ...
  return (
    <div className="flex flex-col min-h-full">
      <Header
        title="Proprietăți"
        subtitle={`${properties.length} anunțuri indexate · Făgăraș + 20km rază`}
      />
      ...
      <div className="p-6 space-y-4">
        ...
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          <span className="font-semibold text-foreground">{filtered.length}</span> anunțuri găsite
          {ownerFilter === "PF" && (
        ...
        {filtered.length === 0 && (
          <div className="text-center py-16 text-muted-foreground">
            ...
            <p className="text-sm">Niciun anunț găsit pentru filtrele selectate.</p>
          </div>
        )}
      </div>
    </div>
